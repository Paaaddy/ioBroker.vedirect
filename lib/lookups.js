 ...
 * @returns {(rawKey: string | undefined) => string}
 */
function makeLookup(table, valueKey, unknownLabel) {
	return function lookup(rawKey) {
		const entry = rawKey !== undefined ? table[rawKey] : undefined;
...
const lookups = {
	...
	// cap_ble is defined here but currently unused (no CAP_BLE field in stateAttr).
	// Included for completeness per VE.Direct protocol spec.
